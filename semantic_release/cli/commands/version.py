...
def version(  # noqa: C901
    ...
    * Create a release (if supported) in the remote VCS for this tag
    ...
    with Repo(str(runtime.repo_dir)) as git_repo:
        tag_to_versions = tags_and_versions(git_repo.tags, translator)

    # If the new version has already been released, we fail and abort if strict;
    # otherwise we exit with 0.
    if new_version in {v for _, v in tag_to_versions}:
        if opts.strict:
            ctx.fail(
        ...
        if template_dir.is_dir():
            if opts.noop:
                noop_report(
                    ...
                    f"{template_dir!r} relative to {runtime.repo_dir!r}. "
                ...
                all_paths_to_add.extend(
                    recursive_render(
                        template_dir, environment=env, _root_dir=str(runtime.repo_dir)
                    )
                )
