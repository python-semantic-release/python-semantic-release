...
class RepositoryTests(TestCase):
    ...
    @mock.patch("semantic_release.repository.config.get", wrapped_config_get(upload_to_repository=False))
    def test_upload_enabled_with_upload_to_repository_false(self):
        self.assertFalse(ArtifactRepo.upload_enabled())

    @mock.patch("semantic_release.repository.config.get", wrapped_config_get(upload_to_pypi=False))
    def test_upload_enabled_with_upload_to_pypi_false(self):
        self.assertFalse(ArtifactRepo.upload_enabled())

    @mock.patch.dict("os.environ", {"PYPI_USERNAME": "pypi-username", "PYPI_PASSWORD": "pypi-password"})
    ...
    @mock.patch.dict("os.environ", {"REPOSITORY_USERNAME": "repo-username", "REPOSITORY_PASSWORD": "repo-password"})
    ...
    def test_repo_with_token_only(self):
        repo = ArtifactRepo(Path("dist"))
        self.assertIn("Providing only password or token without username is deprecated", self._caplog.messages)
    ...
    def test_repo_without_pypirc_file(self, mock_path_exists):
        ...
        self.assertEqual(str(cm.exception), "Missing credentials for uploading to artifact repository")
    ...
    @mock.patch.object(ArtifactRepo, "_handle_credentials_init")
    @mock.patch("semantic_release.repository.config.get", wrapped_config_get(dist_glob_patterns="*.tar.gz,*.whl"))
    def test_repo_with_custom_dist_globs(self, mock_handle_creds):
        repo = ArtifactRepo(Path("dist"))
        self.assertEqual(repo.dists, ["dist/*.tar.gz", "dist/*.whl"])

    @mock.patch.object(ArtifactRepo, "_handle_credentials_init")
    @mock.patch("semantic_release.repository.config.get",
    ...
    @mock.patch.object(ArtifactRepo, "_handle_credentials_init")
    @mock.patch("semantic_release.repository.config.get", wrapped_config_get(repository="testpypi"))
    def test_repo_with_repo_name_testpypi(self, mock_handle_creds):
        repo = ArtifactRepo(Path("dist"))
        self.assertEqual(repo.repository_name, "testpypi")

    @mock.patch.object(ArtifactRepo, "_handle_credentials_init")
    @mock.patch("semantic_release.repository.config.get", wrapped_config_get(repository="invalid"))
    ...
    @mock.patch.object(ArtifactRepo, "_handle_credentials_init")
    @mock.patch("semantic_release.repository.config.get", wrapped_config_get(repository_url="https://custom-repo"))
