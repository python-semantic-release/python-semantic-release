...
from semantic_release.changelog.context import create_pypi_url
...
def test_release_notes_context_release_url_filter(
    ...
    with mock.patch.dict(os.environ, {}, clear=True):
        ...
        # render_release_notes normalizes line endings to os.linesep, so expected
        # must use os.linesep (\r\n on Windows) to match the actual output.
        expected_content = str.join(
            os.linesep,
            [
                f"[{version.as_tag()}]({hvcs_client.create_release_url(version.as_tag())})",
                "",
            ],
        )
...
@pytest.mark.parametrize("hvcs_client", [Github, Gitlab, Gitea, Bitbucket])
def test_default_release_notes_template_w_pypi_link(
    example_git_https_url: str,
    hvcs_client: type[Github | Gitlab | Gitea | Bitbucket],
    artificial_release_history: ReleaseHistory,
    today_date_str: str,
    ...
    """
    Given a release history with multiple releases, when generating release notes with
    include_pypi_link=True, then the PyPi registry section is appended to the output.
    """
    released_versions = iter(artificial_release_history.released.keys())
    version = next(released_versions)
    prev_version = next(released_versions)
    ...
    # The PyPi registry section is appended when include_pypi_link=True
    pypi_url = create_pypi_url(hvcs.repo_name, str(version))
    # For Gitea (no Detailed Changes), Block1 ends with os.linesep and the
    # template's {{ "\n" }} produces a blank line before ---, so we need the
    # leading "" to add the extra os.linesep separator.
    # For non-Gitea the Detailed Changes block already ends with os.linesep,
    # and the template's {{ "\n" }} outputs a single newline before --- (no
    # blank line), so we must NOT include the leading "" to avoid doubling it.
    pypi_section_lines: list[str] = (
        [
            "",
            "---",
            "",
            ...
            "",
            f"- [PyPi Registry]({pypi_url})",
            "",
        ]
        if isinstance(hvcs, Gitea)
        else [
            "---",
            "",
            "**Installable artifacts are available from**:",
    ...
    expected_content += str.join(os.linesep, pypi_section_lines)
