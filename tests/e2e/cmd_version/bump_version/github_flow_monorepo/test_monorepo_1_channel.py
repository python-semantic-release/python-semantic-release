...
def test_githubflow_monorepo_rebuild_1_channel(
    ...
    target_repo_definition = build_monorepo_w_github_flow_w_default_release_channel(
    ...
    for curr_release_key, steps in release_versions_2_steps.items():
        ...
        for step in steps[::-1]:  # reverse order
            if step["action"] == RepoActionStep.CHANGE_DIRECTORY:
                ...
                if not release_directory.is_relative_to(mirror_repo_dir):
                    release_directory = mirror_repo_dir
